...
void TcpServer::EventLoop() {
  ...
  for (int i = 0; i < kThreadsNum; ++i) {
    if (pthread_create(&tid, nullptr, WorkerThread, reinterpret_cast<void*>(this))) {
  ...
  while(1) {
    ...
    task_que_.produce(clnt_sfd);
  }
}

void* TcpServer::WorkerThread(void* args) {
  pthread_detach(pthread_self());

  auto this_ptr = static_cast<TcpServer*>(args);
  int sfd = this_ptr->task_que_.consume();
  this_ptr->HandleIoEvent(sfd);
